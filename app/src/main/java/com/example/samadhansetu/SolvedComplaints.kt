...
class SolvedComplaints : AppCompatActivity() {
    ...
    private fun setupRecyclerView() {
        ...
        binding.rvSolvedComplaints.apply { // Bind to the correct RecyclerView ID
            layoutManager = LinearLayoutManager(this@SolvedComplaints)
    ...
    private fun fetchStudentComplaints() {
        ...
        database.orderByChild("regNo").equalTo(regNum).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    for (complaintSnapshot in snapshot.children) {
                        ...
                        if (complaint != null && complaint.state.equals("Solved", ignoreCase = true)) {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@SolvedComplaints, "Failed to load complaints: ${error.message}", Toast.LENGTH_LONG).show()
            }
        })
    }
